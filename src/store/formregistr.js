...
export default {
  ...
  actions: {
    ...
    //TODO сделать нормальную валидацию, при ошибке сервера изменить сообщение об ошибке
    onSubmit: (store, router) => {
      if (
        store.state.fields[0].value.length > 1 ||
        store.state.fields[1].value.length > 1
      ) {
        //элементарная проверка что поля не пустые
        store.commit("validChange", true);
        makeRequest(
          `users/login?email=${store.state.fields[0].value}&password=${store.state.fields[1].value}`,
          {
            method: "POST",
            headers: {
